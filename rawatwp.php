...
if ( 'rawatwp/rawatwp.php' !== plugin_basename( __FILE__ ) ) {
	if ( is_admin() ) {
		...
		add_action(
			'admin_notices',
			static function() {
				echo '<div class="notice notice-error"><p>RawatWP nonaktif otomatis karena folder plugin tidak valid. Gunakan folder plugin: <code>rawatwp</code>.</p></div>';
			}
		);
	}
	return;
